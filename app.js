...
d3.csv("breitbartData.csv").then((data) => {
            ...
            function update(data_count, data) {
                ...
                var yAxis = d3.scaleLinear().domain([0, d3.max([data_count, (d) => d.count]).range([height - margin.bottom, margin.top]);

                        svg.selectAll(".y-axis").transition().duration(3000).call(d3.axisLeft(yAxis));
